...
def main(env_name):
  ...
  def policy_params_fn(num_steps, make_policy, params, eval_it=0, eval_env=None):
      ...
      if eval_it % 1 == 0:
          ...
          # state = jax.jit(eval_env.reset)(rng=subkey[None,:])
          # state = jax.jit(eval_env.reset)(rng=subkey)
          state = eval_env.reset(rng=subkey)
          ...
          for i in range(150):
              ...
              state = eval_env.step(state, action)   # JIT-safe step
              frame = eval_env.render(state, height=240, width=320)#, camera='skew_side')
              frames.append(frame)
  ...
  policy_params_fn = functools.partial(policy_params_fn, eval_env=env)
