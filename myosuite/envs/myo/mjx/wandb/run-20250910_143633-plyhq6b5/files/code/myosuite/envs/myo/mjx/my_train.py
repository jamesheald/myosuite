...
def main(env_name):
  ...
  eval_env = wrapper.wrap_for_brax_training(env, episode_length=100, action_repeat=1)
  ...
  def policy_params_fn(num_steps, make_policy, params, eval_it=0, eval_env=None):
      ...
      if eval_it % 1 == 0:
          ...
          frames.append(frame)
          for _ in range(eval_env._max_steps):
              key, subkey = jax.random.split(key)
              action, _ = policy(state.obs, subkey[None, :])
              state = jit_env_step(state, action)   # JIT-safe step
              frame = eval_env.render(state, height=240, width=320, camera='skew_side', backend='egl')
              frames.append(frame)
