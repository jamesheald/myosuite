...
class MjxReachEnvV0(mjx_env.MjxEnv):
    def __init__(
        ...
        # Optionally, you can delete the temp file after loading
        os.remove(tmp_path)
    ...
    def step(self, state: State, action: jp.ndarray) -> State:
        ...
        truncation = jp.where(state.info['my_step'] >= self._max_steps, 1. - done, jp.array(0.))
        
        my_step = jp.where(jp.logical_or(done, truncation), jp.array(0, dtype=jp.int32), state.info['my_step'])
        ...
        solved_now = 1.*(reach_dist<self.near_th)
        solved = jp.maximum(solved_now, state.info['solved'])
        solved_metric = jp.where(jp.logical_or(done, truncation), solved, 0.)
        ...
        state.metrics.update(
            ...
            solved=solved_metric,
            solved_frac=solved_now/self._max_steps
        )
