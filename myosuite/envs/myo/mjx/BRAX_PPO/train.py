...
from myosuite.envs.myo.mjx.BRAX_PPO import losses as ppo_losses
from myosuite.envs.myo.mjx.BRAX_PPO import ppo_networks as ppo_networks
...
def train(
    ...
    save_final_checkpoint_path: Optional[str] = None,
    restore_checkpoint_path: Optional[str] = None,
    restore_params: Optional[Any] = None,
    restore_value_fn: bool = True,
    run_evals: bool = True,
    keep_prob: float = 1.0,
    dynamics_update: str = None,
):
  """PPO training.
  ...
  Args:
    ...
    dynamics_update: before dynamics model 'before' or 'after' updating the policy
    precoder_state
  ...
  key_policy, key_value, key_dynamics = jax.random.split(global_key, 3)
  del global_key
  ...
  make_policy = ppo_networks.make_inference_fn(ppo_network, precoder_state)
  ...
  def training_step(
    ...
    key_sgd, dynamics_key_sgd, key_generate_unroll, new_key = jax.random.split(key, 4)
  ...
  policy_params_fn(current_step, make_policy, params, 0)
  ...
  for it in range(num_evals_after_init):
    ...
    policy_params_fn(current_step, make_policy, params, it)
  ...
  if save_final_checkpoint_path is not None:
    ckpt_config = checkpoint.network_config(
        observation_size=obs_shape,
        action_size=env.action_size,
        normalize_observations=normalize_observations,
    ...
    checkpoint.save(
        save_final_checkpoint_path, current_step, params, ckpt_config
    )
